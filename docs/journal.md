...
> Facts and real outputs only — never fabricated numbers. Newest entry first.
...
- Returns headline stats for Landing page stats strip
- `filings_processed` (365,387), `financial_facts` (851,731), `flag_events` (2,746)
- `companies_scored` (9,047), `flag_types` (4), `calibration_day` (34 today)
- Pure datetime `_trading_days()` — no numpy, INF-018 consistency standard
- Inserted BEFORE static file mount (learned from data-inventory routing bug)
